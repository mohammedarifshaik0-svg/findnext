...
export function PortfolioMiniPreview({ data }: { data: PreviewData }) {
  ...
  if (data.theme === "canvas") return <div className="relative min-h-[650px] overflow-hidden bg-[var(--portfolio-bg)] p-7 text-[var(--portfolio-text)]" style={style}>
    ...
    <div className="relative mt-8 grid grid-cols-2 gap-2">{skills(data).map((skill, index) => <div key={skill.id} className="rounded-lg border border-white/10 p-3" style={{ backgroundColor: `${palette.colors[index % 3]}22` }}><p className="text-[10px] font-semibold">{skill.title}</p><div className="mt-3 h-0.5" style={{ backgroundImage: `linear-gradient(90deg, ${palette.colors[0]}, ${palette.colors[2]})` }} /></div>)}</div>
  ...
  if (data.theme === "ledger") return <div className="relative min-h-[650px] overflow-hidden bg-[var(--portfolio-bg)] p-7 text-[var(--portfolio-text)]" style={style}>
    ...
    <p className="mt-14 text-[9px] tracking-[.2em] text-[#829579]">[ PROFILE / ACTIVE ]</p><h2 className="mt-5 text-4xl font-bold leading-[.95] tracking-[-.055em]">{data.headline || data.fullName || "Professional story"}</h2>{data.headline && <p className="mt-4 text-xs text-[#7e7e86]">{data.fullName}</p>}<p className="mt-7 line-clamp-5 text-xs leading-5 text-[#7e7e86]">{data.professionalSummary || "Your professional story will appear here."}</p>
    <div className="mt-12 grid grid-cols-[75px_1fr] gap-5 border-y border-[#1c1c1f] py-7"><p className="text-[8px] tracking-[.16em] text-[#829579]">CAPABILITY</p><div>{skills(data).map((skill, index) => <div key={skill.id} className="flex justify-between border-b border-[#1c1c1f] py-2 first:pt-0"><span className="text-[10px]">{skill.title}</span><span className="text-[8px] text-[#444]">0{index + 1}</span></div>)}</div></div>
    <div className="mt-9"><p className="text-[8px] tracking-[.16em] text-[#829579]">CHRONOLOGY</p>{data.experiences.slice(0, 2).map((row) => <div key={row.id} className="mt-4 grid grid-cols-[1fr_2fr] gap-3"><p className="text-[9px] text-[#555]">record</p><div><p className="text-xs font-semibold">{row.company || "Company"}</p><p className="text-[10px] text-[#777]">{row.role || "Role"}</p></div></div>)}</div>
    </div>
  </div>;
  ...
  return <div className="relative min-h-[650px] overflow-hidden bg-[var(--portfolio-bg)] p-7 text-[var(--portfolio-text)] [font-family:Georgia,serif]" style={style}>
    ...
    <div className="mt-14 text-center"><p className="text-[8px] tracking-[.25em] text-[#dfba86]">A PROFESSIONAL STORY</p><h2 className="mt-5 text-5xl font-light leading-[.88] tracking-[-.04em]">{data.fullName || "Your name"}</h2><p className="mt-4 text-sm italic text-[#dfba86]">{data.headline || "Professional headline"}</p></div>
    <div className="mt-10 grid min-h-48 grid-cols-[.8fr_1.2fr] overflow-hidden rounded-lg border border-[#dfba86]/15 bg-[#101735]"><div className="grid place-items-center bg-[radial-gradient(circle_at_20%_20%,rgba(223,186,134,.35),transparent_35%),radial-gradient(circle_at_80%_80%,rgba(79,70,229,.35),transparent_40%),#111833]"><span className="text-5xl text-[#dfba86]/60">{initials(data.fullName)}</span></div><div className="flex flex-col justify-center bg-[#090e22]/70 p-5"><p className="text-base leading-tight">The record of what you changed.</p><p className="mt-4 line-clamp-4 text-[10px] leading-4 text-[#8d96b0]">{data.professionalSummary || "Your professional story will appear here."}</p></div></div>
    <div className="mt-10"><p className="text-[8px] tracking-[.2em] text-[#dfba86]">03 / PROFESSIONAL RECORD</p>{data.experiences.slice(0, 2).map((row, index) => <div key={row.id} className="mt-4 grid grid-cols-[70px_1fr] border-t border-[#dfba86]/10 pt-3"><p className="text-[8px] text-[#dfba86]">CH. 0{index + 1}</p><div><p className="text-xs">{row.role || "Role"}</p><p className="mt-1 text-[9px] text-[#8d96b0]">{row.company || "Company"}</p></div></div>)}</div>
    </div>
  </div>;
}
