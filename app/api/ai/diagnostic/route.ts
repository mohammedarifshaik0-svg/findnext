...
async function run() {
  ...
  try {
    const result = await generateText({ model: "inclusionai/ling-3.0-flash-sante-free", instructions: "Improve grammar only. Return only the revised text. Do not add facts.", prompt: "I am a student learning software engineering and building small projects.", maxOutputTokens: 900, maxRetries: 0, abortSignal: AbortSignal.timeout(25000) });
    stage = "verification";
    const verification = await generateText({ model: "inclusionai/ling-3.0-flash-sante-free", instructions: "Return PASS if the candidate preserves the source facts, otherwise FAIL.", prompt: JSON.stringify({source:"I am a student learning software engineering and building small projects.",candidate:result.text}), maxOutputTokens: 20, maxRetries: 0, abortSignal: AbortSignal.timeout(25000) });
