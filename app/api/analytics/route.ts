...
export async function GET() {
  ...
  const {data:rows,error}=await createAdminClient().from("portfolio_views").select("viewed_at,visitor_hash,referrer_domain,device_class").eq("profile_id",userId).gte("viewed_at",since).order("viewed_at");
  ...
  return Response.json({totalViews:rows?.length??0,uniqueVisitors:visitors.size,days:[...days].map(([date,views])=>({date,views})),sources:[...sources].sort((a,b)=>b[1]-a[1]).slice(0,5).map(([source,views])=>({source,views})),devices:[...devices].map(([device,views])=>({device,views}))});
}
