...
import { BriefcaseBusiness, Check, ChevronRight, CircleDollarSign, Eye, FileText, GraduationCap, LayoutTemplate, Link2, Loader2, Plus, Save, ShieldCheck, Sparkles, Trash2, Upload, UserRound } from "lucide-react";
...
const templates = [
  { id: "studio", name: "Studio", description: "Editorial, spacious and quietly confident.", swatch: "from-indigo-500 via-violet-400 to-cyan-300" },
  { id: "canvas", name: "Canvas", description: "Bold, dark and built for visible impact.", swatch: "from-[#07111f] via-indigo-900 to-cyan-600" },
  { id: "ledger", name: "Ledger", description: "Structured, professional and recruiter-friendly.", swatch: "from-[#12233f] via-slate-600 to-amber-400" },
] as const;
    ...
    <div className="mx-auto grid max-w-[1500px] gap-6 p-4 sm:p-6 xl:grid-cols-[240px_minmax(0,1fr)_380px]">
      ...
      <section className="min-w-0 space-y-5">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}><TabsList className="sr-only"><TabsTrigger value="profile">Profile</TabsTrigger><TabsTrigger value="experience">Experience</TabsTrigger><TabsTrigger value="education">Education</TabsTrigger><TabsTrigger value="extras">More</TabsTrigger><TabsTrigger value="templates">Templates</TabsTrigger><TabsTrigger value="publish">Publish</TabsTrigger><TabsTrigger value="plans">Plans</TabsTrigger></TabsList>
          ...
          <TabsContent value="templates"><Section title="Choose your portfolio style" description="Every template is included. Change it whenever you like without losing any content."><div className="grid gap-4 md:grid-cols-3">{templates.map((template) => <button key={template.id} onClick={() => chooseTemplate(template.id)} disabled={saving} className={`group overflow-hidden rounded-2xl border-2 text-left transition ${data.theme === template.id ? "border-indigo-600 shadow-[0_12px_30px_rgba(79,70,229,.16)]" : "border-slate-200 hover:border-slate-300"}`}><div className={`h-36 bg-gradient-to-br ${template.swatch} p-4`}><div className="h-full rounded-lg bg-white/90 p-3 shadow-sm"><div className="h-2 w-14 rounded bg-slate-800/80" /><div className="mt-3 h-1.5 w-24 rounded bg-slate-300" /><div className="mt-5 grid grid-cols-[.45fr_1fr] gap-2"><div className="h-14 rounded bg-slate-200" /><div className="space-y-2"><div className="h-2 rounded bg-slate-300" /><div className="h-2 rounded bg-slate-200" /><div className="h-2 w-2/3 rounded bg-slate-200" /></div></div></div></div><div className="bg-white p-4"><div className="flex items-center justify-between"><p className="font-semibold">{template.name}</p>{data.theme === template.id && <span className="grid h-6 w-6 place-items-center rounded-full bg-indigo-600 text-white"><Check className="h-4 w-4" /></span>}</div><p className="mt-1 text-sm leading-5 text-slate-500">{template.description}</p></div></button>)}</div><div className="mt-6 flex justify-end"><Button variant="outline" onClick={preview} disabled={saving}><Eye className="h-4 w-4" />Open full preview</Button></div></Section></TabsContent>
