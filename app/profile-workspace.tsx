...
export function ProfileWorkspace({ account }: { account: { name: string; email: string } }) {
  ...
  if (loading) return <main className="vxl-studio-loading"><VxlLogo compact className="vxl-studio-logo" /><Loader2 className="h-5 w-5 animate-spin" /><span>Preparing your studio</span></main>;
  return <main className={`vxl-workspace ${uiTheme === "dark" ? "is-dark" : "is-light"}`}>
    <header className="vxl-studio-header"><div className="vxl-studio-lockup"><VxlLogo className="vxl-studio-logo" /><p className="vxl-studio-caption">Portfolio studio</p></div><div className="vxl-studio-actions"><span className="hidden text-xs md:inline">{savedAt ? `Saved at ${new Date(savedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : "Draft not saved yet"}</span><button className="vxl-studio-icon" onClick={() => { const next = uiTheme === "dark" ? "light" : "dark"; setUiTheme(next); window.localStorage.setItem("vxl_ui_theme", next); }} aria-label={`Switch to ${uiTheme === "dark" ? "light" : "dark"} theme`}>{uiTheme === "dark" ? <Sun/> : <Moon/>}</button><Button variant="outline" onClick={preview} disabled={saving}><Eye className="h-4 w-4" /><span>Preview</span></Button><Button className="vxl-studio-primary" onClick={save} disabled={saving}>{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}<span>Save draft</span></Button></div></header>
