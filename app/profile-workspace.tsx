...
export function ProfileWorkspace({ account }: { account: { name: string; email: string } }) {
  ...
  if (loading) return <main className="grid min-h-screen place-items-center bg-[#f5f5f7]"><Loader2 className="h-7 w-7 animate-spin text-[#5f3cfe]" /></main>;
  return <main className="min-h-screen bg-[#f5f5f7] text-[#12121d]">
    <header className="sticky top-0 z-20 border-b border-zinc-200/80 bg-white/90 backdrop-blur-xl"><div className="mx-auto flex h-16 max-w-[1500px] items-center justify-between px-4 sm:px-6"><div className="flex items-center gap-3"><div className="vxl-mark">X</div><div><p className="text-base font-extrabold tracking-[-.04em]">VXL</p><p className="text-xs text-zinc-500">Portfolio studio</p></div></div><div className="flex items-center gap-2 sm:gap-3"><span className="hidden text-sm text-zinc-500 md:inline">{savedAt ? `Saved ${new Date(savedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : "Not saved yet"}</span><Button variant="outline" onClick={preview} disabled={saving}><Eye className="h-4 w-4" />Preview</Button><Button className="bg-gradient-to-r from-[#18181b] to-[#09090b]" onClick={save} disabled={saving}>{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Save</Button></div></div></header>
    <div className="mx-auto grid max-w-[1500px] gap-6 p-4 sm:p-6 xl:grid-cols-[240px_minmax(0,1fr)_380px]">
      <aside className="vxl-sidebar rounded-2xl border p-4 shadow-xl shadow-black/10 xl:sticky xl:top-22 xl:h-[calc(100vh-7rem)]"><div className="rounded-xl border border-white/10 bg-white/[.06] p-4 text-white"><div className="flex items-center justify-between"><span className="text-sm font-medium">Profile strength</span><strong>{completion}%</strong></div><Progress value={completion} className="mt-3 bg-white/15" /><p className="mt-3 text-xs leading-5 text-zinc-400">Complete profiles make stronger portfolios.</p></div><nav className="mt-4 grid grid-cols-2 gap-2 xl:grid-cols-1">{[{ id: "profile", icon: UserRound, label: "Personal profile" }, { id: "experience", icon: BriefcaseBusiness, label: "Experience" }, { id: "education", icon: GraduationCap, label: "Education" }, { id: "extras", icon: Sparkles, label: "Skills & more" }, { id: "templates", icon: LayoutTemplate, label: "Templates" }, { id: "publish", icon: Link2, label: "Publish settings" }, { id: "plans", icon: CircleDollarSign, label: "Plans & referrals" }].map(({ id, icon: Icon, label }) => <button key={id} onClick={() => setActiveTab(id)} className={`nav-item ${activeTab === id ? "active" : ""}`}><Icon className="h-4 w-4" />{label}<ChevronRight className="ml-auto hidden h-4 w-4 xl:block" /></button>)}</nav><div className="mt-4 border-t border-white/10 pt-4"><div className="flex items-center gap-2 text-sm font-medium text-white"><ShieldCheck className="h-4 w-4 text-emerald-400" />Private by default</div><p className="mt-2 text-xs leading-5 text-zinc-400">Your portfolio stays private until you publish it.</p><a className="mt-3 block text-xs font-semibold text-violet-300" href="mailto:findnext@ignyxx.in">findnext@ignyxx.in</a></div></aside>
