  ...
  return (
    <div className="flex flex-wrap gap-3">
      {Boolean(profile.email) && (
        <a className={`inline-flex min-h-11 items-center gap-2 rounded-full border px-5 text-sm font-semibold transition ${palette}`} href={`mailto:${value(profile.email)}`}>
...
function ContactLine({ profile, mutedClass }: { profile: Row; mutedClass: string }) {
  return (
    <div className={`flex flex-wrap gap-x-6 gap-y-2 text-sm ${mutedClass}`}>
      ...
      {Boolean(profile.email) && (
        <a className="inline-flex items-center gap-2 hover:underline" href={`mailto:${value(profile.email)}`}>
          <Mail className="h-4 w-4" />
          {value(profile.email)}
        </a>
      )}
...
function ExternalLinks({ links, className }: { links: Row[]; className: string }) {
  ...
  return (
    <div className="flex flex-wrap gap-x-5 gap-y-3">
      {links.map((link) => (
        <a className={`inline-flex items-center gap-1.5 text-sm font-semibold ${className}`} key={value(link.id)} href={value(link.url)} target="_blank" rel="noreferrer">
...
function EditorialTemplate({ data }: { data: PortfolioData }) {
  ...
  return (
    <main className="portfolio-surface min-h-screen overflow-hidden bg-[var(--portfolio-bg)] text-[var(--portfolio-text)] [font-family:'Avenir_Next','Helvetica_Neue',Arial,sans-serif]" style={portfolioStyle("studio", value(profile.accent), Number(profile.effect_intensity ?? 65), value(profile.text_tone))}>
      ...
      <nav className="mx-auto flex max-w-[1440px] items-center justify-between px-6 py-7 sm:px-10 lg:px-20">
        <a href="#top" className="text-xl italic text-[#dfba86] [font-family:'Iowan_Old_Style','Baskerville',Georgia,serif]">
          {name || "Portfolio"}
        </a>
        <div className="hidden items-center gap-8 text-xs font-semibold tracking-[.14em] text-[#8d96b0] md:flex">
          <a href="#story">STORY</a>
          <a href="#experience">RECORD</a>
          <a href="#work">WORK</a>
        </div>
        {Boolean(profile.email) && (
          <a href={`mailto:${value(profile.email)}`} className="rounded-full border border-[#dfba86]/50 px-5 py-2.5 text-xs font-semibold tracking-[.12em] text-[#dfba86]">
            CONTACT
          </a>
        )}
      </nav>
      ...
      <header id="top" className="relative mx-auto max-w-[1440px] px-6 pb-24 pt-16 text-center sm:px-10 lg:px-20 lg:pb-36 lg:pt-24">
        ...
        <div className={`relative mt-14 overflow-hidden rounded-xl border border-[#dfba86]/20 bg-[#101735] text-left shadow-[0_40px_120px_rgba(0,0,0,.45)] ${profile.photo_path ? "grid lg:grid-cols-[1.35fr_.65fr]" : "grid lg:grid-cols-[.82fr_1.18fr]"}`}>
          <div className={`relative min-h-[360px] overflow-hidden ${profile.photo_path ? "bg-[#111833]" : "bg-[radial-gradient(circle_at_24%_20%,rgba(223,186,134,.35),transparent_25%),radial-gradient(circle_at_80%_80%,rgba(79,70,229,.32),transparent_35%),linear-gradient(135deg,#0b1025,#171e42)]"}`}>
            ...
            {!profile.photo_path && (
              <div className="absolute inset-0 grid place-items-center">
                <span className="text-[9rem] font-light text-[#dfba86]/65 [font-family:'Iowan_Old_Style','Baskerville',Georgia,serif]">{initials(name)}</span>
              </div>
            )}
          ...
          <div className="flex flex-col justify-center border-t border-[#dfba86]/15 bg-[#090e22]/75 p-8 backdrop-blur-xl sm:p-12 lg:border-l lg:border-t-0 lg:p-14">
            ...
            <div className="mt-8">
              <PortfolioActions profile={profile} tone="gold" />
            </div>
          </div>
        </div>
      </header>
      ...
      <section id="story" className="border-y border-[#dfba86]/15 bg-[#0d1430] px-6 py-24 sm:px-10 lg:px-20 lg:py-32">
        <div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-[.65fr_1.35fr]">
          <div>
            ...
            <h2 className="mt-5 max-w-md text-5xl font-light leading-[1.02] [font-family:'Iowan_Old_Style','Baskerville',Georgia,serif]">The work behind the person.</h2>
          </div>
          <div>
            <p className="whitespace-pre-line text-xl leading-9 text-[#e9e3df]">{value(profile.professional_summary)}</p>
            <div className="mt-8">
              <ContactLine profile={profile} mutedClass="text-[#8d96b0]" />
            </div>
          </div>
        </div>
      </section>
        ...
        <section className="mx-auto max-w-[1440px] px-6 py-24 sm:px-10 lg:px-20 lg:py-32">
          ...
          <div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {skills.map((skill, index) => (
              <div key={value(skill.id)} className="group border border-[#dfba86]/16 bg-[#101735] p-7 transition hover:-translate-y-1 hover:border-[#dfba86]/45">
                <p className="text-[11px] tracking-[.18em] text-[#dfba86]">0{index + 1}</p>
                <h3 className="mt-7 text-lg font-semibold">{value(skill.title)}</h3>
                <div className="mt-5 h-px bg-gradient-to-r from-[#dfba86] to-transparent" />
              </div>
            ))}
          </div>
        </section>
      ...
      {experiences.length > 0 && (
        <section id="experience" className="bg-[#111a3d] px-6 py-24 sm:px-10 lg:px-20 lg:py-32">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mt-14 divide-y divide-[#dfba86]/15 border-y border-[#dfba86]/15">
              {experiences.map((row, index) => (
                <article key={value(row.id)} className="grid gap-7 py-10 lg:grid-cols-[180px_1fr]">
                  <div>
                    <span className="text-xs tracking-[.12em] text-[#dfba86]">CHAPTER {String(index + 1).padStart(2, "0")}</span>
                    <p className="mt-3 text-sm text-[#8d96b0]">{years(row)}</p>
                  </div>
                  <div>
                    <p className="text-sm font-semibold tracking-[.08em] text-[#dfba86]">
                      {value(row.company)}
                      {row.location ? ` · ${value(row.location)}` : ""}
                    </p>
                    <h3 className="mt-2 text-3xl font-light [font-family:'Iowan_Old_Style','Baskerville',Georgia,serif]">{value(row.role)}</h3>
                    {Boolean(row.description) && (
                      <ul className="mt-5 grid gap-3 text-[#aab1c5]">
                        {lines(row.description).map((line) => (
                          <li className="flex gap-3 leading-7" key={line}>
                            <span className="mt-3 h-px w-4 shrink-0 bg-[#dfba86]" />
                            {line}
                          </li>
                        ))}
                      </ul>
      ...
      <section id="work" className="mx-auto max-w-[1440px] px-6 py-24 sm:px-10 lg:px-20 lg:py-32">
        {projects.length > 0 && (
          <>
            ...
            <h2 className="mt-4 text-5xl font-light [font-family:'Iowan_Old_Style','Baskerville',Georgia,serif]">Ideas made tangible</h2>
            <div className="mt-14 grid gap-6 md:grid-cols-3">
              {projects.map((project, index) => (
                <article key={value(project.id)} className="overflow-hidden border border-[#dfba86]/15 bg-[#101735]">
                  <div className="relative h-48 overflow-hidden bg-[radial-gradient(circle_at_20%_20%,rgba(223,186,134,.35),transparent_30%),radial-gradient(circle_at_80%_70%,rgba(99,102,241,.45),transparent_35%),#090e22]">
                    <span className="absolute bottom-4 right-5 text-7xl font-light text-white/10 [font-family:'Iowan_Old_Style','Baskerville',Georgia,serif]">0{index + 1}</span>
                  </div>
                  <div className="p-7">
        ...
        {achievements.length > 0 && (
          <div className="mt-24 grid gap-5 md:grid-cols-2">
            {achievements.map((achievement) => (
              <blockquote key={value(achievement.id)} className="border border-[#dfba86]/15 bg-[#0d1430] p-8 text-2xl italic leading-9 text-[#e9e3df] [font-family:'Iowan_Old_Style','Baskerville',Georgia,serif]">
        ...
        <section className="border-y border-[#dfba86]/15 bg-[#0d1430] px-6 py-24 sm:px-10 lg:px-20">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mt-10 grid gap-5 md:grid-cols-2">
              {education.map((row) => (
                <article key={value(row.id)} className="border border-[#dfba86]/15 bg-[#090e22] p-7">
                  <p className="text-xs tracking-[.12em] text-[#dfba86]">{years(row)}</p>
                  <h3 className="mt-4 text-2xl [font-family:'Iowan_Old_Style','Baskerville',Georgia,serif]">{value(row.qualification)}</h3>
                  <p className="mt-2 text-[#8d96b0]">{value(row.institution)}</p>
                </article>
              ))}
              {extras.map((row) => (
                <article key={value(row.id)} className="border border-[#dfba86]/15 bg-[#090e22] p-7">
                  <p className="text-xs tracking-[.12em] text-[#dfba86]">{value(row.item_type).toUpperCase()}</p>
                  <h3 className="mt-4 text-2xl [font-family:'Iowan_Old_Style','Baskerville',Georgia,serif]">{value(row.title)}</h3>
                  <p className="mt-2 text-[#8d96b0]">{value(row.subtitle)}</p>
                </article>
              ))}
            </div>
          </div>
...
function PrismTemplate({ data }: { data: PortfolioData }) {
  ...
  const name = value(profile.full_name);
  return (
    <main className="portfolio-surface min-h-screen overflow-hidden bg-[var(--portfolio-bg)] text-[var(--portfolio-text)] [font-family:'Avenir_Next','Helvetica_Neue',Arial,sans-serif]" style={portfolioStyle("canvas", value(profile.accent), Number(profile.effect_intensity ?? 65), value(profile.text_tone))}>
      <div className="portfolio-atmosphere" />
      ...
      <nav className="sticky top-0 z-20 border-y border-white/10 bg-[#050508]/80 px-6 py-5 backdrop-blur-2xl sm:px-10 lg:px-20">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <span className="text-lg font-black">{initials(name)} / VXL</span>
          ...
          <ExternalLinks links={links} className="text-slate-300 hover:text-white" />
        </div>
      </nav>
      ...
      <section id="identity" className="relative border-b border-white/8 px-6 py-24 sm:px-10 lg:px-20 lg:py-32">
        ...
        <div className="relative mx-auto max-w-7xl">
          ...
          <h2 className="mt-4 max-w-4xl text-4xl font-bold tracking-[-.035em] sm:text-6xl">A career built through clarity, curiosity and measurable change.</h2>
          <div className="mt-14 grid gap-10 lg:grid-cols-[1fr_1fr]">
            <blockquote className="border-l-4 border-violet-500 pl-7 text-3xl font-light leading-snug sm:text-4xl">“The strongest work turns complexity into momentum.”</blockquote>
            <div>
              <p className="whitespace-pre-line text-lg leading-8 text-slate-400">{value(profile.professional_summary)}</p>
              <div className="mt-7">
                <ContactLine profile={profile} mutedClass="text-slate-500" />
              </div>
            </div>
          </div>
        </div>
      ...
      {skills.length > 0 && (
        <section className="relative px-6 py-24 sm:px-10 lg:px-20 lg:py-32">
          ...
          <div className="relative mx-auto max-w-7xl">
            ...
            <h2 className="mt-4 text-4xl font-bold tracking-[-.035em] sm:text-5xl">The toolkit behind the outcomes</h2>
            <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {skills.map((skill, index) => (
                <article key={value(skill.id)} className={`rounded-2xl border border-white/10 p-6 shadow-2xl ${index % 3 === 0 ? "bg-gradient-to-br from-teal-500/20 to-blue-500/5" : index % 3 === 1 ? "bg-gradient-to-br from-violet-500/20 to-pink-500/5" : "bg-gradient-to-br from-slate-500/15 to-emerald-500/5"}`}>
                  <div className="flex items-center justify-between">
                    <h3 className="text-xl font-semibold">{value(skill.title)}</h3>
                    <span className="text-xs text-slate-500">0{index + 1}</span>
                  </div>
                  <div className="mt-7 h-1 overflow-hidden rounded-full bg-white/8">
                    <div className="h-full w-[78%] bg-gradient-to-r from-teal-400 via-violet-500 to-pink-500" />
                  </div>
                </article>
              ))}
            </div>
      ...
      {experiences.length > 0 && (
        <section id="trajectory" className="border-y border-white/8 bg-[radial-gradient(circle_at_82%_28%,rgba(236,72,153,.16),transparent_32%),linear-gradient(180deg,#08060e,#0d0710)] px-6 py-24 sm:px-10 lg:px-20 lg:py-32">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="relative mt-14 space-y-12 before:absolute before:bottom-4 before:left-[7px] before:top-4 before:w-px before:bg-gradient-to-b before:from-teal-400 before:via-violet-500 before:to-pink-500">
              {experiences.map((row) => (
                <article key={value(row.id)} className="relative grid gap-4 pl-10 lg:grid-cols-[190px_1fr]">
                  <span className="absolute left-0 top-2 h-3.5 w-3.5 rounded-full border-4 border-[#0d0710] bg-violet-400 shadow-[0_0_25px_rgba(167,139,250,.8)]" />
                  <p className="text-sm text-slate-500">{years(row)}</p>
                  <div>
                    <h3 className="text-2xl font-semibold">{value(row.role)}</h3>
        ...
        <section id="systems" className="px-6 py-24 sm:px-10 lg:px-20 lg:py-32">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mt-12 grid gap-6 md:grid-cols-3">
              {projects.map((project, index) => (
                <article key={value(project.id)} className={`relative min-h-[360px] overflow-hidden rounded-2xl border border-white/10 p-7 ${index % 3 === 0 ? "bg-gradient-to-br from-cyan-400 via-blue-500 to-violet-700" : index % 3 === 1 ? "bg-gradient-to-br from-pink-500 via-rose-500 to-orange-300" : "bg-gradient-to-br from-emerald-400 via-teal-500 to-blue-700"}`}>
                  <div className="absolute inset-0 bg-[linear-gradient(130deg,transparent,rgba(255,255,255,.22),transparent)] opacity-60" />
      ...
      {achievements.length > 0 && (
        <section className="relative border-y border-white/8 px-6 py-24 text-center sm:px-10 lg:px-20">
          ...
          <div className="relative mx-auto max-w-4xl">
            ...
            {achievements.map((achievement) => (
              <p key={value(achievement.id)} className="mt-4 text-2xl font-light leading-9 text-slate-200 sm:text-3xl">
                {value(achievement.title)}
              </p>
            ))}
          </div>
        ...
        <section className="px-6 py-24 sm:px-10 lg:px-20">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mt-10 grid gap-4 md:grid-cols-2">
              {education.map((row) => (
                <article key={value(row.id)} className="border border-white/10 bg-white/[.025] p-7">
                  <p className="text-xs text-slate-500">{years(row)}</p>
                  <h3 className="mt-4 text-xl font-semibold">{value(row.qualification)}</h3>
                  <p className="mt-2 text-slate-400">{value(row.institution)}</p>
                </article>
              ))}
            </div>
          </div>
      ...
      <footer id="contact" className="relative px-6 py-28 text-center sm:px-10 lg:py-40">
        ...
        <div className="relative">
          <p className="text-xs uppercase tracking-[.25em] text-slate-500">The next chapter</p>
          <h2 className="mx-auto mt-5 max-w-3xl text-5xl font-black tracking-[-.045em] sm:text-7xl">Let’s make it matter.</h2>
          <div className="mt-9 flex justify-center">
            <PortfolioActions profile={profile} tone="dark" />
          </div>
...
function ZenTemplate({ data }: { data: PortfolioData }) {
  ...
  const name = value(profile.full_name);
  return (
    <main className="portfolio-surface min-h-screen bg-[var(--portfolio-bg)] text-[var(--portfolio-text)] [font-family:'Helvetica_Neue',Arial,sans-serif]" style={portfolioStyle("ledger", value(profile.accent), Number(profile.effect_intensity ?? 65), value(profile.text_tone))}>
      <div className="portfolio-atmosphere" />
            ...
            {projects.map((project, index) => (
              <article key={value(project.id)} className="group grid items-center gap-8 px-6 py-12 transition hover:bg-[#080808] sm:px-10 lg:grid-cols-[1fr_.72fr] lg:px-20">
                ...
                <div className="relative h-48 overflow-hidden border border-[#1c1c1f] bg-[linear-gradient(135deg,#0c0c0d,#020202)]">
                  ...
                  <span className="absolute bottom-5 right-6 text-7xl font-bold text-white/[.06]">0{index + 1}</span>
                </div>
              </article>
            ))}
          </div>
