...
import { PortfolioTemplate } from "./portfolio-templates";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return { title: `${data.data.profile.full_name} — ${data.data.profile.headline || "Portfolio"}`, description: String(data.data.profile.professional_summary || "Professional portfolio on VXL").slice(0, 160) };
}
